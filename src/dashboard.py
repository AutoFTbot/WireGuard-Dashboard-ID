...
def get_conf_peer_key(config_name):
    ...
    try:
        ...
        return config_name + " tidak sedang berjalan. Aktifkan itu."
...
def auth_req():
    ...
    if req == "true":
        if '/static/' not in request.path and \
            ...
            print("Pengguna tidak masuk - Coba untuk mengakses:" + str(request.endpoint))
            if request.endpoint != "index":
                session['message'] = "Silakan masuk terlebih dahulu."
            else:
                session['message'] = ""
            conf.clear()
...
def auth():
    ...
    config.clear()
    return jsonify({"status": False, "msg": "Nama pengguna atau kata sandi salah."})
...
def update_acct():
    ...
    if len(request.form['username']) == 0:
        session['message'] = "Nama pengguna tidak boleh kosong."
    ...
    try:
        ...
        config.clear()
        session['message'] = "Nama pengguna berhasil diperbarui!"
        session['message_status'] = "success"
        session['username'] = request.form['username']
        return redirect(url_for("settings"))
    except Exception:
        session['message'] = "Perbarui nama pengguna gagal."
        session['message_status'] = "danger"
        config.clear()
        return redirect(url_for("settings"))
...
def update_peer_default_config():
    ...
    if len(request.form['peer_endpoint_allowed_ip']) == 0 or \
        ...
        session['message'] = "Silakan lengkapi bidang yang diperlukan"
        session['message_status'] = "danger"
        config.clear()
        return redirect(url_for("settings"))
    # Check DNS Format
    ...
    if not check_DNS(dns_addresses):
        session['message'] = "Format DNS pengaturan tidak benar."
        session['message_status'] = "danger"
        config.clear()
        return redirect(url_for("settings"))
    ...
    if not check_Allowed_IPs(ip):
        session['message'] = "Format Endpoint Allowed IPs tidak benar." \
                             "Contoh: 192.168.1.1/32 or 192.168.1.1/32,192.168.1.2/32"
        session['message_status'] = "danger"
        config.clear()
        return redirect(url_for("settings"))
    # Check MTU Format
    if not len(request.form['peer_mtu']) > 0 or not request.form['peer_mtu'].isdigit():
        session['message'] = "Format MTU tidak benar."
        session['message_status'] = "danger"
        config.clear()
        return redirect(url_for("settings"))
    # Check keepalive Format
    if not len(request.form['peer_keep_alive']) > 0 or not request.form['peer_keep_alive'].isdigit():
        session['message'] = "Format Persistent keepalive tidak benar."
        session['message_status'] = "danger"
        config.clear()
        return redirect(url_for("settings"))
    # Check peer remote endpoint
    if not check_remote_endpoint(request.form['peer_remote_endpoint']):
        session['message'] = "Format Peer Remote Endpoint tidak benar. " \
                             "Alamat IP atau domain yang valid (tanpa http:// atau https://)."
        session['message_status'] = "danger"
        config.clear()
        return redirect(url_for("settings"))
    ...
    try:
        set_dashboard_conf(config)
        session['message'] = "Pengaturan default berhasil diperbarui!"
        session['message_status'] = "success"
        config.clear()
        return redirect(url_for("settings"))
    except Exception:
        session['message'] = "Perbarui pengaturan default gagal."
        session['message_status'] = "danger"
        config.clear()
        return redirect(url_for("settings"))
...
def update_pwd():
    ...
    if hashlib.sha256(request.form['currentpass'].encode()).hexdigest() == config.get("Account", "password"):
        if hashlib.sha256(request.form['newpass'].encode()).hexdigest() == hashlib.sha256(
            ...
            try:
                set_dashboard_conf(config)
                session['message'] = "Password berhasil diperbarui!"
                session['message_status'] = "success"
                config.clear()
                return redirect(url_for("settings"))
            except Exception:
                session['message'] = "Perbarui password gagal."
                session['message_status'] = "danger"
                config.clear()
                return redirect(url_for("settings"))
        else:
            session['message'] = "Password baru Anda tidak cocok."
            session['message_status'] = "danger"
            config.clear()
            return redirect(url_for("settings"))
    else:
        session['message'] = "Password saat ini Anda tidak cocok."
        session['message_status'] = "danger"
        config.clear()
        return redirect(url_for("settings"))
...
def update_wg_conf_path():
    ...
    set_dashboard_conf(config)
    config.clear()
    session['message'] = "Perbarui jalur konfigurasi WireGuard berhasil!"
    session['message_status'] = "success"
    subprocess.Popen('bash wgd.sh restart', shell=True)
...
def add_peer(config_name):
    ...
    if len(public_key) == 0 or len(dns_addresses) == 0 or len(allowed_ips) == 0 or len(endpoint_allowed_ip) == 0:
        return "Silakan lengkapi bidang yang diperlukan."
    if not isinstance(keys, list):
        return config_name + " tidak sedang berjalan. Aktifkan itu."
    if public_key in keys:
        return "Kunci publik sudah ada."
    check_dup_ip = g.cur.execute(
        "SELECT COUNT(*) FROM " + config_name + " WHERE allowed_ip LIKE '" + allowed_ips + "/%'", ) \
        .fetchone()
    if check_dup_ip[0] != 0:
        return "Allowed IPs sudah digunakan oleh pengguna lain."
    if not check_DNS(dns_addresses):
        return "Format DNS tidak benar. Contoh: 1.1.1.1"
    if not check_Allowed_IPs(endpoint_allowed_ip):
        return "Format Endpoint Allowed IPs tidak benar."
    if len(data['MTU']) == 0 or not data['MTU'].isdigit():
        return "Format MTU tidak benar."
    if len(data['keep_alive']) == 0 or not data['keep_alive'].isdigit():
        return "Format Persistent Keepalive tidak benar."
    try:
        if enable_preshared_key:
...
def save_peer_setting(config_name):
    ...
    if peer:
        ...
        if not check_IP_with_range(endpoint_allowed_ip):
            return jsonify({"status": "failed", "msg": "Format Endpoint Allowed IPs tidak benar."})
        if not check_DNS(dns_addresses):
            return jsonify({"status": "failed", "msg": "Format DNS tidak benar. Contoh: 1.1.1.1"})
        if len(data['MTU']) == 0 or not data['MTU'].isdigit():
            return jsonify({"status": "failed", "msg": "Format MTU tidak benar."})
        if len(data['keep_alive']) == 0 or not data['keep_alive'].isdigit():
            return jsonify({"status": "failed", "msg": "Format Persistent Keepalive tidak benar."})
        if private_key != "":
        ...
        return jsonify({"status": "failed", "msg": "Pengguna ini tidak ada."})
...
def init_dashboard():
    ...
    if "password" not in config['Account']:
        config['Account']['password'] = 'Mantap123'
