...
app_official_name="WireGuard Dashboard ID"
...
help() {
  display_logo
  printf "${YELLOW}=================================================================================\n"
  printf "${YELLOW}+     ${BLUE}<Wireguard Panel> by AutoFTbot ${BLUE}https://github.com/AutoFTbot        ${YELLOW}+\n"
  printf "${YELLOW}=================================================================================${NC}\n"
...
update_wgd() {
  new_ver=$(python3 -c "import json; import urllib.request; data = urllib.request.urlopen('https://api.github.com/repos/AutoFTbot/WireGuard-Dashboard-ID.git').read(); output = json.loads(data);print(output['tag_name'])")
  printf "%s\n" "$dashes"
  ...
  if [ "$up" = "Y" ]; then
    ...
    git pull https://github.com/AutoFTbot/WireGuard-Dashboard-ID.git $new_ver --force >  /dev/null 2>&1
